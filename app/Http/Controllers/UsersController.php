...
class UsersController extends Controller
{
    public function index(){
      ...
      public function myOrder(){
        $orderlist=Orders_model::where('users_id',Auth::id())->get();
        $cart_datas=array();
        foreach($orderlist as $dataOrd){
          $cart_datas[$dataOrd->id]=Cart_model::where('session_id',$dataOrd->session_id)->get();
          $total_price[$dataOrd->id]=0;
          foreach (  $cart_datas[$dataOrd->id]as $cart_data){
              $total_price[$dataOrd->id]+=$cart_data->price*$cart_data->quantity;
          }
        }
          return view('checkout.my_orders',compact('cart_datas','total_price'));
    }
